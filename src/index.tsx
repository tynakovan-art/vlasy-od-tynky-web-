...
/** Obrázek s fallbackem a lazy-loadem */
function SmartImage({
  srcs,
  alt,
  className,
  ...
  width?: number;
  height?: number;
  priority?: boolean; // hero logo apod.
  ...
  const common = {
    ...
    // fetchPriority jen pro moderní prohlížeče; TS typy to neznají – přidáme přes prop hack:
    ...(!priority ? {} : { fetchPriority: "high" as any }),
    onError: () => setI((v) => v + 1),
  };
  if (i < srcs.length) return <img src={srcs[i]} {...common} />;
  return fallback;
}
...
export default function Site() {
  ...
  const LOGO_TEXT = ["/logo-text.png"];       // Textové „Vlasy od Týnky“
  ...
  /** Akční ceny – přeškrtnutá původní + zvýrazněná akční */
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800">
      {/* NAVBAR */}
      <header className="sticky top-0 z-20 bg-white/80 backdrop-blur border-b border-subtle">
        <div className="mx-auto max-w-6xl px-6 py-4 flex items-center justify-between">
          <a href="#hero" className="flex items-center gap-3">
            <SmartImage
              ...
              priority
              fallback={
                <div className="h-9 w-9 rounded-xl flex items-center justify-center text-white gradient-bg">
      ...
      <section id="hero" className="relative z-10">
        <div className="relative mx-auto max-w-6xl px-6 pt-12 pb-16 grid md:grid-cols-2 gap-10 items-center">
          {/* Levý sloupec */}
          <div>
            <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-white mb-4 gradient-bg">
            ...
            >
              Kadeřnictví
              <span className="block gradient-text">Vlasy od Týnky</span>
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a href={`tel:${PHONE_RAW}`} className="rounded-2xl px-5 py-3 text-white text-sm font-medium shadow-card gradient-bg">
                📞 Zavolat {PHONE_DISPLAY}
              </a>
              <a href="#sluzby" className="rounded-2xl px-5 py-3 text-sm font-medium bg-white/70 backdrop-blur hover:bg-white border border-subtle">
                Prohlédnout služby
              </a>
            </div>
          </div>
          ...
          <div className="relative md:justify-self-end">
            <div className="absolute -inset-6 rounded-[2rem] opacity-20 blur-2xl gradient-bg" />
            <div className="relative rounded-[2rem] border border-subtle bg-white/70 backdrop-blur p-8 shadow-soft text-center overflow-hidden">
              <SmartImage
                srcs={LOGO_TEXT}
                ...
                height={120}
                fallback={
                  <div className="h-16 w-16 mx-auto mb-4 rounded-2xl flex items-center justify-center text-white gradient-bg">
              ...
              <div className="mt-5 flex flex-col gap-2">
                <a href={IG_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm border border-subtle bg-white hover:bg-slate-50">
                  <Instagram className="h-4 w-4" /> Napsat na Instagramu
                </a>
                <a href={FB_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm border border-subtle bg-white hover:bg-slate-50">
                  <Facebook className="h-4 w-4" /> Napsat na Facebooku
                </a>
                <a href={`tel:${PHONE_RAW}`} className="inline-flex items-center justify-center gap-2 rounded-xl px-4 py-2 text-sm text-white gradient-bg">
      ...
      {/* BANNER NAD SLUŽBAMI (karta + glow, bez fullwidth podbarvení) */}
      <section aria-label="Akce" className="relative z-10">
        <div className="mx-auto max-w-6xl px-6">
          <div className="relative">
            <div className="absolute -inset-6 rounded-[2rem] opacity-20 blur-2xl gradient-bg" />
            <div className="relative rounded-[2rem] border border-subtle bg-white/70 backdrop-blur p-5 md:p-6 shadow-soft flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="text-slate-800">
                <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm md:text-lg font-semibold text-white gradient-bg">
              ...
              <div className="flex gap-2 shrink-0">
                <a href={`tel:${PHONE_RAW}`} className="inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm text-white gradient-bg">
                  <Phone className="h-4 w-4 mr-1" /> Zavolat {PHONE_DISPLAY}
                </a>
                <a href="#cenik" className="inline-flex items-center justify-center rounded-xl px-4 py-2 text-sm border border-subtle bg-white hover:bg-slate-50">
                  Mrknout na ceník
                </a>
              </div>
            </div>
      ...
      <section id="sluzby" className="mx-auto max-w-6xl px-6 py-12">
        ...
        <div className="mt-6 grid md:grid-cols-2 gap-6">
          {[
            ...
            <div key={x.t} className="rounded-2xl border border-subtle bg-white/70 p-6">
      ...
      <section id="cenik" className="mx-auto max-w-6xl px-6 py-16">
        {/* banner nahoře – gradient přes celou šířku */}
        <div className="w-full text-white text-center py-4 rounded-xl shadow-card my-6 gradient-bg">
        ...
        <h2 className="text-2xl font-bold flex items-center gap-3">
          Ceník
          <span className="px-3 py-1 rounded-full text-sm font-semibold bg-rose-50 text-rose-600 border border-rose-200">
            –20 % říjen
          </span>
        </h2>

        <div className="relative mt-6 overflow-hidden rounded-2xl border border-subtle bg-white">
          {/* vodoznak */}
          <div aria-hidden className="pointer-events-none absolute inset-0 flex items-center justify-center opacity-10"
            ...
            <SmartImage srcs={LOGO_SILUETY} alt="" className="max-w-[80%] md:max-w-[50%] h-auto scale-110 blur-[1.2px] select-none pointer-events-none" />
          </div>
      ...
      <section id="kontakt" className="mx-auto max-w-6xl px-6 py-16 grid md:grid-cols-2 gap-10 items-start">
        <div>
          ...
          <div className="mt-4 flex flex-col gap-3 text-slate-700">
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              <a href={IG_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm border border-subtle bg-white hover:bg-slate-50">
                <Instagram className="h-4 w-4" /> Instagram
              </a>
              <a href={FB_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm border border-subtle bg-white hover:bg-slate-50">
                <Facebook className="h-4 w-4" /> Facebook
              </a>
              <a href={`tel:${PHONE_RAW}`} className="inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm text-white gradient-bg">
                <Phone className="h-4 w-4" /> Zavolat {PHONE_DISPLAY}
              </a>
            </div>
          </div>
        ...
        <div className="w-full md:justify-self-end">
          <div className="relative max-w-md ml-auto w-full">
            <div className="absolute -inset-6 rounded-[2rem] opacity-20 blur-2xl gradient-bg" />
            <div className="relative rounded-[2rem] border border-subtle bg-white/70 backdrop-blur p-2 shadow-soft overflow-hidden">
              <iframe
                src={MAP_EMBED}
                width="100%"
                height="340"
      ...
      {/* PODĚKOVÁNÍ – vodoznak textového loga, menší aby se vešel pěkně */}
      <section className="relative z-10">
        <div className="relative mx-auto max-w-md px-6 py-12">
          <div className="relative rounded-[2rem] border border-subtle bg-white/70 backdrop-blur py-24 px-10 shadow-soft text-center overflow-hidden">
            <div aria-hidden className="pointer-events-none absolute inset-0 flex items-center justify-center opacity-20"
                 style={{
                   WebkitMaskImage: "radial-gradient(70% 70% at 50% 50%, rgba(0,0,0,1) 55%, rgba(0,0,0,0) 100%)",
                   maskImage:       "radial-gradient(70% 70% at 50% 50%, rgba(0,0,0,1) 55%, rgba(0,0,0,0) 100%)",
                 }}>
              <SmartImage srcs={LOGO_TEXT} alt="" className="max-w-[55%] h-auto blur-[1px] select-none pointer-events-none" />
            </div>
            <div className="relative z-10">
