...
/* Obrázek s fallbackem – maximálně tolerantní, aby build nepadal na typech */
type SmartImageProps = React.ImgHTMLAttributes<HTMLImageElement> & {
  srcs: string[];
  fallback?: React.ReactNode;
};
function SmartImage({ srcs, alt = "", className, fallback, ...rest }: SmartImageProps) {
  // zahodíme případný neplatný atribut 'priority'
  // @ts-expect-error
...
export default function Site() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-800">
      ...
      {/* BANNER NAD SLUŽBAMI – karta se “svatozáří” jako v hero */}
          ...
          <div className="relative">
            ...
            <div className="relative rounded-[2rem] border border-slate-200/60 bg-white/70 backdrop-blur p-5 md:p-6 shadow-xl flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div className="text-slate-800">
                <div className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm md:text-lg font-semibold text-white" style={{ background: grad }}>
